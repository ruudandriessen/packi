...
interface BunLockFile {
  ...
  packages: Record<string, [string, string, { dependencies: Record<string, string> }, string]>;
}
...
export function parseBunLock(lockPath: string): Package[] {
  try {
    ...
    const {
      workspaces,
      packages
    } = JSON5.parse(lockContent) as BunLockFile;

    const installedDependencies = Object.values(workspaces).flatMap((workspace) => {
