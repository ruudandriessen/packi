...
function parseLockV2(lockData: PackageLockJsonV2) {
    const { dependencies, devDependencies } = lockData.packages[""];

    const installedPackages = Object.keys(dependencies).concat(
...
function parseLockV3(lockData: PackageLockJsonV3) {
    ...
    return Object.entries(lockData.packages)
        .filter(([path]) =>
            installedPackages.includes(path.replace("node_modules/", "")),
        )
        .map(([path, info]) => ({
